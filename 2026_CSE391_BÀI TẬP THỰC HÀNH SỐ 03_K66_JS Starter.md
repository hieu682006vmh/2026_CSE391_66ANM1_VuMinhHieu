...
> - Em thấy dòng thông báo nào trong console?
  -trả lời: Hello from JavaScript!
> - Điều này cho em biết JavaScript đang làm gì khi trang web được tải?
  - trả lời: JavaScript chạy file main.js khi trang tải và in thông báo ra Console
...
>
> - HTML: tạo nội dung trang web
> - CSS: tạo giao diện và bố cục
> - JavaScript: tạo tương tác và xử lí logic
...
>   - `number` là: dữ liệu dạng số
>   - `string` là: chuỗi văn bản
...
> Câu hỏi:
> - Dòng log hiển thị gì sau khi em sửa thông tin: Hello from JavaScript!
Xin chào, mình là Minh Hieu, năm nay mình 20 tuổi.
> - Nếu em quên dấu `;` hoặc quên dấu `+`, điều gì xảy ra? Trình duyệt báo lỗi thế nào: Uncaught SyntaxError

---

#### Bước 5: Phản tư nhanh (Reflection)

> - Điều thú vị nhất em vừa khám phá được về console là gì: Console giúp chạy thử JavaScript nhanh
> - Em gặp lỗi cú pháp nào? Em đã xử lý bằng cách nào (tự sửa, hỏi bạn, đọc lỗi, tìm Google, …): em chưa gặp lỗi nào cả, nhưng nếu có em sẽ đọc trong console và sửa lại code
...
> - Trường hợp `score = 9`: Dự đoán vs Thực tế: trùng với dự đoán là giỏi
> - Trường hợp `score = 6`: Dự đoán vs Thực tế: trùng với dự đoán là trung bình
...
- Cấu trúc if/else giống như các ngã rẽ. Chương trình kiểm tra từng điều kiện từ trên xuống, điều kiện nào đúng trước thì thực hiện lệnh ở nhánh đó
...
> - Phần nào trong if/else hoặc hàm khiến em khó hiểu nhất: Phần khó hiểu nhất là cách hàm return giá trị.
> - Em đã làm gì để vượt qua (thử nhiều lần, hỏi bạn, xem lại ví dụ, tra Google, …): Em thử chạy nhiều lần trong Console và quan sát kết quả để hiểu cách hoạt động
...
- Tìm phần tử HTML có id = "status"
- Trả về phần tử đó để JavaScript có thể thao tác.
> - Sự kiện `"click"` xảy ra khi nào:
Người dùng nhấn chuột vào một phần tử (button, div, link,...)
> - Trong đoạn code trên, khi nhấn nút `btnHello`, điều gì thay đổi trên trang: Trang web thay đổi nội dung văn bản bằng JavaScript
...
> Câu hỏi:
> - Sự kiện `"input"` khác gì so với `"click"`: "input" chạy liên tục khi đang gõ.
...
>
> Em hãy:
> - Tự mô tả DOM bằng lời của em: DOM là mô hình biểu diễn trang HTML dưới dạng cây các phần tử, cho phép JavaScript truy cập, chỉnh sửa nội dung, kiểu dáng và sự kiện của trang web.
...
> Câu hỏi:
> - Điểm giống nhau về chức năng giữa 2 đoạn code trên là gì: Cả hai đoạn code đều bắt sự kiện click của nút btnHello và hiển thị thông báo alert khi người dùng nhấn nút.
...
- JS thuần:
    ...
    + Dùng addEventListener() để gắn sự kiện.
- jQuery:
    + Dùng $("#btnHello") để chọn phần tử nhanh hơn.
    + Dùng .on("click") để xử lý sự kiện.
-> jQuery viết ngắn gọn và dễ đọc hơn.
> - Em hãy tra cứu nhanh “What is jQuery used for?” và ghi 2 ý chính:
>   1. Giúp thao tác DOM dễ dàng và viết JavaScript ngắn gọn hơn
>   2. Xử lý sự kiện, hiệu ứng và tương tác trang web nhanh chóng

---

### 6.2. Tự đánh giá \& định hướng

> 1. Sau buổi lab, em tò mò nhất về phần nào của JavaScript/DOM: Phần thao tác DOM và xử lý sự kiện vì có thể thay đổi nội dung trang web trực tiếp
> 2. Em muốn tự làm thêm tính năng gì trên trang web (vd: bộ đếm, đổi theme, pop-up, mini game, …):
Làm chức năng đổi theme sáng/tối (dark mode) cho website, popup chào mừng,...
